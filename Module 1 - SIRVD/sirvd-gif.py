...
parser.add_argument("--vacc", type=float, default=10,
                    help="Vaccination percentage")
args = parser.parse_args()

vaccination_percentage = args.vacc
...
REFRACTORY = 6
REFRACTORY_END = REFRACTORY + 21  # = 27
DEAD = 28
d_rate = 0.05
...
def update(frame):
    ...
    grid = new_grid
    n_sus[frame] = np.sum(grid == SUSCEPTIBLE)
    n_inf[frame] = np.sum((grid >= INFECTED_START) & (grid <= PARA))
    n_dead[frame] = np.sum(grid == DEAD)
    n_refr[frame] = np.sum((grid >= PARA) & (grid < REFRACTORY_END))
    n_vac[frame] = np.sum(grid == VACCINATED)
...
anim.save(f"./simulations/sim_vacc:{vaccination_percentage}.gif", writer=PillowWriter(fps=10))
...
ax.legend()
plt.tight_layout()
plt.savefig(f"./simulations/plot_vacc:{vaccination_percentage}.png", dpi=300)
plt.show()